package com.aemtools.sync.packmgr.uninstall.crx

import com.aemtools.sync.packmgr.uninstall.exceptions.PackageNotUninstalledException
...
class CrxUninstallerPackage(private val url: String) {
  ...
  fun uninstallPackage(packageFileName: String, pkgmgr: PackageManagerHelper, httpClient: CloseableHttpClient?, log: Logger) {
    log.info("Starting to uninstall package $packageFileName...")
    val get = HttpGet("$url.jsp?cmd=rm&name=$packageFileName")

    val xmlResponse = pkgmgr.executePackageManagerMethodXml(httpClient, get)
    val xpath = XPathFactory.instance().compile("""/crx/response/status[@code="200" and .="ok"]""")
