...
object CqComponentPatterns {
     ...
     */
    val cqComponentRootTag = XmlPatterns.xmlTag().withName("jcr:root").with(
        xmlTagWithAttribute(
            "jcr:primaryType",
            CQ_COMPONENT
        )
    ...
    val cqComponentSlingResourceSuperType = psiElement(JpTypes.VALUE_TOKEN)
        .with(
            HostCondition("sling:resourceSuperType value") { _, host, _ ->
                psiElement()
    ...
    val cqComponentComponentGroup = psiElement(JpTypes.VALUE_TOKEN)
        .with(
            HostCondition("componentGroup value") { _, host, _ ->
                psiElement()
    ...
    val cqComponentIsContainer = psiElement(JpTypes.VALUE_TOKEN)
        .with(
            HostCondition("cq:isContainer value") { _, host, _ ->
                psiElement()
    ...
    val cqComponentNoDecoration = psiElement(JpTypes.VALUE_TOKEN)
        .with(
            HostCondition("cq:noDecoration value") { _, host, _ ->
                psiElement()
